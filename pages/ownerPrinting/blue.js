...
const printTemplate = (deviceId, serviceId, characteristicId, arrData) => {
  ...
  for (var i = 0; i < arrData.length; i++) {
    paintingTitles(arrData[i].residential);
    painting('商品名称        ', '        商品数量');
    for (var j = 0; j < arrData[i].product_list.length; j++) {
      ...
      painting(objsss, ' *' + arrData[i].product_list[j].sell_num);
    }
    paintingNums('收货人  :' + arrData[i].name)
    paintingNums('联系方式:' + arrData[i].telephone)
