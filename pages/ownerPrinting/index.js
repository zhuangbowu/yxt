...
Page({
    ...
    connectBluetooth(deviceid, function(data) {
      ...
      getAllservice(deviceid, function(data) {
        ...
        if (data) {
          ...
          // var characteristicId = data.characteristicId;
          // printTemplate(deviceId, serviceId, characteristicId, that.data.arrData);
        } else {
          wx.showModal({
            title: '提示',
  ...
  checkboxChange: function(e) {
    ...
    for (var i = 0; i < this.data.listData.length; i++) {
      // this.data.listData[i].id == arrOrder
      for (var j = 0; j < arrOrder.length; j++) {
        if (this.data.listData[i].id == arrOrder[j]) {
    ...
    for (var i = 0; i < arrData.length; i++) {
      ...
      for (var j = 0; j < arrData[i].product_list.length; j++) {
        if (arrData[i].product_list[j].sell_num !== 0) {
          darrddd.push(arrData[i].product_list[j]);
        }
      }
      arrData[i].product_list = darrddd;
      if (arrData[i].product_list.length !== 0) {
        arrDada.push(arrData[i]);
      }
    }
    this.data.arrData = arrDada;
